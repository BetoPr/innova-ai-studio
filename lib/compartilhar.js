...
async function uploadCover() {
  ...
  // Comprime se acima de 1MB. Imagens de IA gerada costumam vir em 4-9MB e
  // travavam o upload em conexões mais lentas. Compressão pra ~500KB resolve.
  if (origFile.size > 1 * 1024 * 1024) {
    $('globalStatus').textContent = 'Otimizando imagem…';
    try {
      const before = origFile.size;
      const compressed = await compressImage(origFile);
      console.log(`[uploadCover] comprimido: ${(before / 1024).toFixed(0)}KB → ${(compressed.size / 1024).toFixed(0)}KB (${Math.round((1 - compressed.size / before) * 100)}% menor)`);
      origFile = compressed;
    } catch (e) {
      console.warn('[uploadCover] compressão falhou, segue com original:', e.message);
    }
  }
