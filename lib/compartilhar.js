...
async function uploadCover() {
  ...
  // Tenta 2x com path novo a cada tentativa. Timeout de 30s por tentativa.
  // A cada retry, recria o File a partir do Blob pra evitar state stale do SDK.
  let lastErr;
  for (let attempt = 1; attempt <= 2; attempt++) {
    const path = `${state.user.id}/${Date.now()}-${randomSuffix()}.${ext}`;
    // File novo, mesmo conteúdo — alguns SDKs não gostam de reusar File entre tentativas.
    const fileForUpload = new File([origFile], origFile.name, { type: origFile.type });
    const t0 = performance.now();
    console.log(`[uploadCover] tentativa ${attempt} → POST ${path}`);
    try {
      const uploadPromise = supabase.storage.from('prompt-covers').upload(path, fileForUpload, {
        cacheControl: '3600',
        upsert: false,
        contentType: origFile.type,
      });
      const { error } = await withTimeout(uploadPromise, 30000, 'Upload da capa');
      const dt = ((performance.now() - t0) / 1000).toFixed(1);
      if (error) {
        console.error(`[uploadCover] tentativa ${attempt} erro do SDK (${dt}s):`, error);
        throw error;
      }
