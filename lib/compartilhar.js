...
const state = {
  ...
  data: {
    ...
    isOfficial: false,      // só admin pode marcar (vai pra Galeria Oficial)
  },
  editingId: null,
  origSlug: null,
  isPro: false,
...
async function publish() {
  ...
  try {
    ...
    // Só admin pode marcar como Pro / Oficial
    if (state.isAdmin) {
      payload.is_pro = !!state.data.isPro;
      payload.is_official = !!state.data.isOfficial;
    }
...
async function init() {
  ...
  // Sync visibility dos toggles admin
  if (state.isAdmin) {
    $('proToggleCard').hidden = false;
    $('officialToggleCard').hidden = false;
  }
  ...
  $('officialToggle')?.addEventListener('change', e => state.data.isOfficial = e.target.checked);
