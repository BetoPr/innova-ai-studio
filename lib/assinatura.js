...
function fmtDate(iso) {
  if (!iso) return '—';
  // YYYY-MM-DD sem hora vira UTC meianoite -> dia anterior em BR (-3h).
  // Adiciona meio-dia local pra evitar shift de timezone.
...
async function cancelSubscription() {
  ...
  m.innerHTML = `
    <div class="delete-modal-content">
      ...
      <div class="cancel-sub-status" id="cancelSubStatus" style="display:none;"></div>
  ...
  const abortBtn = document.getElementById('cancelSubAbort');
  const confirmBtn = document.getElementById('cancelSubConfirm');
  const statusEl = document.getElementById('cancelSubStatus');

  function close() {
    m.remove();
    document.removeEventListener('keydown', escHandler);
  }
  function escHandler(e) { if (e.key === 'Escape' && !confirmBtn.disabled) close(); }
  document.addEventListener('keydown', escHandler);
  abortBtn.onclick = close;
  m.onclick = (e) => { if (e.target === m && !confirmBtn.disabled) close(); };

  confirmBtn.onclick = async () => {
    abortBtn.disabled = true;
    confirmBtn.disabled = true;
    confirmBtn.innerHTML = '<span class="gal-spinner"></span> Cancelando…';
    statusEl.style.display = 'none';
    statusEl.textContent = '';

    try {
      const { data: { session }, error: sessErr } = await supabase.auth.getSession();
      if (sessErr) throw new Error('Sessão: ' + sessErr.message);
      ...
      confirmBtn.innerHTML = '<i data-lucide="check"></i> Cancelado';
      if (window.lucide) lucide.createIcons();
      showToast('Assinatura cancelada ✓');
      setTimeout(() => location.reload(), 800);
    } catch (err) {
      console.error('[cancel-sub] erro:', err);
      statusEl.textContent = 'Erro: ' + (err.message || err);
      statusEl.style.display = 'block';
      abortBtn.disabled = false;
      confirmBtn.disabled = false;
      confirmBtn.innerHTML = '<i data-lucide="x-circle"></i> Tentar de novo';
      if (window.lucide) lucide.createIcons();
    }
  };
}
...
async function init() {
  ...
  if (prof?.asaas_customer_id) {
    ...
    // Sobrescreve "Próxima cobrança" com a fatura PENDENTE mais próxima
    // (asaas é fonte da verdade, banco pode estar desatualizado se o webhook
    // PAYMENT_CREATED não chegou ainda).
    if (isPro) {
      const upcoming = payments
        .filter(p => p.status === 'PENDING' || p.status === 'OVERDUE')
        .sort((a, b) => new Date(a.dueDate) - new Date(b.dueDate))[0];
      if (upcoming) {
        $('nextDue').textContent = fmtDate(upcoming.dueDate);
      } else {
        // Sem fatura pendente: já pagou tudo até agora, próxima vai ser
        // gerada pelo Asaas no próximo ciclo.
        $('nextDue').textContent = 'Aguardando próximo ciclo';
      }
    }
  }
}
