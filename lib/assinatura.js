...
async function init() {
  ...
  if (isPro) {
    ...
    // "Atualizar forma de pagamento" abre WhatsApp do suporte com email
    // pré-preenchido. Solução MVP — automatizar troca de cartão exige fluxo
    // dedicado com tokenização Asaas; fica pra depois.
    const updateBtn = $('updatePayment');
    if (updateBtn) {
      const msg = `Olá, quero atualizar a forma de pagamento da minha assinatura Innova. Email cadastrado: ${user.email}`;
      updateBtn.href = `https://wa.me/5581991594716?text=${encodeURIComponent(msg)}`;
      updateBtn.target = '_blank';
      updateBtn.rel = 'noopener';
    }
  } else {
